...
function AnalyzeCore(databaseFolder) {
	...
	function _analyzeOneLine(versionNumber, line, fileName, lineNo, needCheckTimeRange) {
		...
		if (isCodingRecord) {
			let lastCodingEnd = resultObject.total['lastCodingEnd'];

			if (lastCodingEnd === 0) {
				resultObject.total['lastCodingEnd'] = endTime;
				workingTime = howLong;
			} else {
				if (endTime - lastCodingEnd < (45 * 60 * 1000)) {
					workingTime = endTime - lastCodingEnd;
					// howLong = endTime - lastCodingEnd;
				}
			}
		...
		resultObject.total['watching'] += howLong;
		resultObject.total['coding'] += workingTime;

		//groupByRule
		if (groupByRules.hasComputer)
			addOneRecordToAGroupBy(resultGroupBy.computer, cols[6]/*computer*/, howLong, isCodingRecord, workingTime);

		if (groupByRules.hasProject)
			addOneRecordToAGroupBy(resultGroupBy.project, cols[5]/*project*/, howLong, isCodingRecord, workingTime);

		if (groupByRules.hasFile)
			addOneRecordToAGroupBy(resultGroupBy.file, cols[4]/*fileName*/, howLong, isCodingRecord, workingTime);

		if (groupByRules.hasLanguage)
			addOneRecordToAGroupBy(resultGroupBy.language, cols[3]/*languageId*/, howLong, isCodingRecord, workingTime);

		if (groupByRules.hasVCS)
			addOneRecordToAGroupBy(resultGroupBy.vcs, vcs ? vcs.join(':') : '::', howLong, isCodingRecord, workingTime);
	 ...
	 */
	function addOneRecordToAGroupBy(context, name, howLong, isCodingRecord, working = 0) {
		if (!name)
			name = 'unknown';
		if (context[name]) {
			context[name][isCodingRecord ? 'coding2' : 'watching'] += howLong;
			context[name]['coding'] += working
		}
		else
			context[name] = {
				coding2: isCodingRecord ? howLong : 0,
				watching: isCodingRecord ? 0 : howLong,
				coding: working
			};
	}
